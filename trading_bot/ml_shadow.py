...
REMAINING_EXTRA = [
    "PRIX_OUVERTURE", "PERF_OUV_SIGNAL",
    "PLUS_HAUT_AVANT_SIGNAL", "PERF_MAX_AVANT_SIGNAL",
]
...
class MLShadowScorer:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            if REMAINING_PATH.exists():
                ...
                self.remaining = loaded.get("model") if isinstance(loaded, dict) else loaded
    ...
    def predict(self, features: dict[str, Any]) -> dict[str, float | str]:
        ...
        try:
            if self.top3 is not None:
                ...
                result["proba_top3_ml"] = round(
                    float(self.top3.predict_proba(self._frame(features, cols))[0, 1]) * 100,
                    2,
                )
            if self.remaining is not None:
                cols = TOP3_NUMERIC + REMAINING_EXTRA + CATEGORICAL
                result["proba_potentiel_1pct_ml"] = round(
                    float(self.remaining.predict_proba(self._frame(features, cols))[0, 1]) * 100,
                    2,
                )
        except Exception as exc:
        ...
        if top3 != "" and remaining != "":
            ...
            result["score_ml_combine"] = round(0.35 * float(top3) + 0.65 * float(remaining), 2)
        elif remaining != "":
